class Translator {
  ...
  async _getResource(lang) {
    ...
    const translation = await this._fetch(`${this._options.filesLocation}/${lang}.json`);
  ...
  async getTranslationByKey(lang, key) {
    ...
    if (typeof key != 'string') throw new Error(`Expected a string for the key parameter, got ${typeof key} instead.`);
  ...
  _translate(translation) {
    ...
    const replace = (element) => {
      ...
      const properties = nullSafeSplit(element.getAttribute('data-i18n-attr'), ' ') || ['innerHTML'];
